...
def geraPlanilhas(aulas, horarios, pasta):
    ...
    for turma in turmas:
        ...
        escala_doc = escala_doc.rename(columns={2:'Segunda', 3:'Terça', 4:'Quarta', 5:'Quinta', 6:'Sexta'})

        escala_doc.to_csv('Resultados/'+pasta+'/Turma-'+turma+'.csv', sep=',')

    sugestoes = hr.geraHorarios(aulas, horarios)
    resultados = aulas[:, [0, 6]]
    resultados = np.column_stack([resultados, np.array(sugestoes).reshape(-1, 1)])
    df_resultados = pd.DataFrame(resultados, columns=['MATÉRIAS', 'CORES', 'SUGESTÃO'])
    # gera um arquivo csv com os horários de cada matéria
    df_resultados.to_csv("Resultados/"+pasta+"/dados.csv", sep=',', index=False)
